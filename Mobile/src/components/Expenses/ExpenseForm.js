...
export default function ExpenseForm({ form, insetsStyle, isPremium, isScanning, onImportReceipt, title }) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.BG }]} contentContainerStyle={[styles.content, insetsStyle]} keyboardShouldPersistTaps="handled">
      <ScreenBackHeader title={title || t("expenseForm.save")} />
      ...
      <View style={[styles.sectionContainer, { backgroundColor: colors.CARD, borderColor: colors.CARD_BORDER, shadowColor: colors.SHADOW_COLOR || "#000" }]}>
        <Text style={[styles.sectionTitle, { color: colors.ACTION_EXPENSE || "#F97316" }]}>{t("expenseForm.transactionInfo")}</Text>

        <Text style={[styles.amountLabel, { color: colors.TEXT_SECONDARY }]}>{t("expenseForm.amount")}</Text>
        <TextInput
        ...
        <Text style={[styles.label, { color: colors.TEXT }]}>{t("expenseForm.name")}</Text>
        <TextInput
        ...
        <PickDateField label={t("expenseForm.date")} value={form.date} onChange={form.setDate} />
      </View>

      {/* SECTION 2: DANH MỤC & HŨ */}
      <View style={[styles.sectionContainer, { backgroundColor: colors.CARD, borderColor: colors.CARD_BORDER, shadowColor: colors.SHADOW_COLOR || "#000" }]}>
        <Text style={[styles.sectionTitle, { color: colors.ACTION_EXPENSE || "#F97316" }]}>{t("expenseForm.categorySection")}</Text>
        ...
        <CategoryGridSelector
          ...
          highlighted
          hintText={t("expenseForm.categoryHint")}
          placeholder={t("expenseForm.categoryPlaceholder")}
          emptyText={t("expenseForm.categoryEmpty")}
        />

        <Text style={[styles.label, { color: colors.TEXT, marginTop: 16, marginBottom: 8 }]}>{t("expenseForm.jarLabel")}</Text>
        {form.jarsLoading ? (
          <Text style={[styles.mutedText, { color: colors.TEXT_MUTED }]}>{t("expenseForm.jarLoading")}</Text>
        ) : form.jars.length === 0 ? (
          <Text style={[styles.mutedText, { color: colors.TEXT_MUTED }]}>{t("expenseForm.jarEmpty")}</Text>
        ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <View style={styles.jarInfoText}>
                      ...
                      <Text style={[styles.jarBalance, { color: colors.TEXT_SECONDARY }]} numberOfLines={1}>{t("expenseForm.jarRemaining")}{formatMoney(getJarBalanceAmount(jar))}</Text>
                    </View>
                  </Pressable>
                );
              })}
      ...
      <View style={[styles.sectionContainer, { backgroundColor: colors.CARD, borderColor: colors.CARD_BORDER, shadowColor: colors.SHADOW_COLOR || "#000" }]}>
        <Text style={[styles.sectionTitle, { color: colors.ACTION_EXPENSE || "#F97316" }]}>{t("expenseForm.noteSection")}</Text>
        ...
        {form.splitInfo?.splits?.length > 0 && (
          <View style={[styles.splitBanner, { backgroundColor: colors.INFO_LIGHT, borderColor: colors.INFO }]}>
            <Text style={[styles.splitTitle, { color: colors.INFO }]}>{t("expenseForm.splitDetected")}</Text>
        ...
        >
          {isScanning ? (
            <View style={styles.importBannerInner}>
              ...
              <Text style={[styles.importBannerText, { color: colors.TEXT_SECONDARY }]}>{t("expenseForm.scanning")}</Text>
            </View>
          ) : (
            <View style={styles.importBannerInner}>
              ...
              <View style={styles.importBannerBody}>
                <Text style={[styles.importBannerTitle, { color: colors.ACTION_EXPENSE || "#F97316" }]}>{t("expenseForm.importFromAI")}</Text>
                <Text style={[styles.importBannerSub, { color: colors.TEXT_MUTED }]}>{t("expenseForm.importSubtitle")}{!isPremium ? t("expenseForm.premiumBadge") : ""}</Text>
              </View>
      ...
      <Pressable style={[styles.saveButton, { backgroundColor: colors.ACTION_EXPENSE || "#F97316" }, form.submitting && styles.saveButtonDisabled]} onPress={form.onSave} disabled={form.submitting}>
        <Text style={styles.saveButtonText}>{form.submitting ? t("expenseForm.saving") : t("expenseForm.save")}</Text>
      </Pressable>
    </ScrollView>
  );
}
