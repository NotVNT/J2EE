...
**Main Reference**: See [`.github/copilot-instructions.md`](.github/copilot-instructions.md) for detailed instructions.
...
1. **Always validate subscription tiers** before allowing restricted features
2. **Use DTO layer** - Never expose entities directly in API responses
3. **Stateless authentication** - JWT tokens contain all necessary claims
4. **Service layer abstraction** - All external APIs (PayOS, Gemini, Brevo) accessed via services
5. **Error handling** - Comprehensive exception handling with meaningful messages
6. **Security first** - Input validation, no hardcoded secrets, proper CORS configuration

---

## Important Files

- **Architecture Rules**: `.claude/rules/` (14 detailed guides)
- **Global Instructions**: `.claude/instructions/global-rules.md`
- **Agents**: `.claude/agents/` (implement-code, reviewer, researcher)
- **Environment Setup**: `.env.example` (template), `.env.example` (DO NOT commit)
- **Detailed Copilot Guide**: `.github/copilot-instructions.md`
...
// Token automatically attached by http service
```

---

**For detailed guidance**: See `.github/copilot-instructions.md`
