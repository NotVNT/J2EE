...
const Menubar = ({ activeMenu, isVip }) => {
  ...
  return (
    <div
      className={`flex items-center justify-between gap-5 border border-b backdrop-blur-[2px] py-4 px-4 sm:px-7 sticky top-0 z-30 ${
        isVip
    ...
    >
      ...
      <div className="relative flex items-center gap-3" ref={dropdownRef}>
        ...
        >
          {user?.profileImageUrl ? (
            ...
            <User
              className={`w-6 h-6 ${isVip ? "text-amber-400" : "text-gray-500"}`}
            />
          )}
        </button>
        ...
        {showDropdown && (
          <div
            className={`vip-dropdown absolute right-0 mt-2 w-52 rounded-lg shadow-lg border py-1 z-50 top-full ${
              isVip
          ...
          >
            <div
              className={`px-4 py-3 border-b vip-dropdown-border ${
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {user?.profileImageUrl ? (
                    ...
                    <User
                      className={`w-4 h-4 ${isVip ? "text-amber-400" : "text-purple-600"}`}
                    />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-sm font-medium truncate vip-dropdown-name ${
                  ...
                  <p
                    className={`text-xs truncate vip-dropdown-email ${
            ...
            <div className="py-1">
              <button
                ...
                className={`vip-dropdown-item flex items-center gap-3 w-full px-4 py-2 text-sm transition-colors duration-150 ${
                  isVip
                    ? "text-amber-100/80 hover:bg-amber-500/8"
                    : "text-gray-700 hover:bg-gray-50"
                }`}
              >
                <User
                  className={`w-4 h-4 vip-dropdown-icon ${isVip ? "text-amber-400" : "text-gray-500"}`}
                />
      ...
      {openSideMenu && (
        <div
          className={`vip-mobile-sidebar fixed top-[73px] left-0 right-0 border-b lg:hidden z-20 ${
            isVip
              ? "bg-[#1a1625] border-amber-500/12"
              : "bg-white border-gray-200"
          }`}
        >
