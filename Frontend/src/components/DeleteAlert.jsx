...
const DeleteAlert = ({ content, onDelete, onCancel }) => {
  ...
  return (
    <div className="flex flex-col gap-5 md:flex-row md:items-start md:gap-5">
      ...
      <div className="relative flex items-center justify-center w-12 h-12 rounded-full bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 shrink-0 mx-auto md:mx-0">
        <span className="absolute inset-0 rounded-full bg-rose-500/15 animate-halo-pulse" />
        <AlertTriangle className="text-rose-500 z-10" size={20} />
      </div>

      <div className="flex-1 text-center md:text-left">
        <p className="text-[13.5px] text-slate-600 dark:text-slate-450 leading-relaxed whitespace-pre-line">
          {content}
        </p>
        ...
        <div className="flex justify-center md:justify-end gap-3 mt-6">
          {onCancel && (
            <button
              ...
              disabled={loading}
              type="button"
              className="px-5 py-2.5 rounded-xl text-[13px] font-semibold transition duration-150 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer
