...
const Sidebar = ({ activeMenu, isVip }) => {
  ...
  return (
    <div
      className={`w-64 h-[calc(100vh-61px)] p-5 sticky top-[61px] z-20 ${
    ...
    >
      <div className="flex flex-col items-center justify-center gap-3 mt-3 mb-7">
        {user?.profileImageUrl ? (
          <div
            className={`relative ${isVip ? "animate-[vip-float_3s_ease-in-out_infinite]" : ""}`}
          >
            <img
          ...
          <User
            className={`w-20 h-20 text-xl ${isVip ? "text-amber-400" : ""}`}
          />
        )}
        <h5
          className={`font-medium leading-6 ${isVip ? "vip-user-name" : "text-gray-950"}`}
        >
          {user.fullName || ""}
        </h5>
        {isVip && (
